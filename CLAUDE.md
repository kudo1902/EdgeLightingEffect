...
- [`docs/review-findings.md`](docs/review-findings.md) - open defects and rough edges, visual ones with offscreen repros. Check here before assuming a behaviour is intended. Nine review passes, all landed. The seventh (I28-I32), eighth (I33-I34) and ninth (I35-I38) cover the threaded-model branch; the ninth is a read of it after the other two landed, and two of its items finish a fix that had reached three call sites out of four. Read I28 **and I38** before touching re-initialisation (anything whose rebuild is input-gated must be invalidated there or it silently does not recover), and I29, I34 **and I36** before adding an `AnimationGroup` override (a group writes no config field, so a value parked on it alone drives nothing).
