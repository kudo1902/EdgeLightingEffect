...
- [`docs/review-findings.md`](docs/review-findings.md) - open defects and rough edges, visual ones with offscreen repros. Check here before assuming a behaviour is intended. Ten review passes, all landed. The seventh (I28-I32) through tenth (I39-I41) cover the threaded-model branch, each one a read of the tree after the previous one's fixes - which is how two of them caught a fix that had reached only three of its four call sites, and one caught a regression the pass before it introduced. Read I28 **and I38** before touching re-initialisation (anything whose rebuild is input-gated must be invalidated there or it silently does not recover); I29, I34 **and I36** before adding an `AnimationGroup` override (a group writes no config field, so a value parked on it alone drives nothing); and **I40** before touching `Config::operator==` or adding a float to the C ABI (one NaN in the config makes it unequal to itself forever, which fires every renderer's `OnConfigChanged` on every frame).
...
- [`docs/threaded-model-design.md`](docs/threaded-model-design.md) - the data-side / render-side split: why `Update` publishes a `ConfigSnapshot` instead of calling renderers, what the C ABI's lock covers and what it deliberately does not, and the five steps it landed in. **Section 6 is the flow charts** - one frame single-threaded and the same frame across three threads, with the blocking edges marked; start there. The same two flows are rendered as interactive diagrams in [`docs/threaded-model-single-thread.html`](docs/threaded-model-single-thread.html) and [`docs/threaded-model-threads.html`](docs/threaded-model-threads.html). Read before touching anything in `lib/capi` that takes a handle.
...
Per-frame contract, and note it splits the class in two. **`Update(dt)` is the DATA SIDE and touches no GL**: it ticks the clock, advances every attached animation by the clock delta, rebuilds the active config, and **publishes a `ConfigSnapshot`** (active config + `clockTime` + raw `rawAccumulatedTime` + a generation counter) into a `ConfigSnapshotBuffer`. `SetConfig` publishes too, but only when the composite actually moved. **`Render(w, h)` is the RENDER SIDE and owns everything GL**: it acquires the newest snapshot, calls `OnConfigChanged` when the generation moved, calls the renderer `Update` pass once per newly acquired snapshot, then draws. So `OnConfigChanged` still fires only on a real change and renderers can rely on that, but it now fires **once** per frame from `Render` rather than up to twice from `SetConfig` + `Update`. Two consequences worth knowing: rendering more than once per update is safe and cheap (the extra draws reuse the held snapshot without re-notifying or re-ticking, which the demo's capture path relies on), and `rawAccumulatedTime` is published **absolute** so a render side that misses snapshots still advances colour cross-fades by exactly the elapsed time. Renderers are independent visual layers and composite by blending - enable any subset. See [`docs/threaded-model-design.md`](docs/threaded-model-design.md) for why the seam exists.
...
- `NeonRenderer` - the neon stroke. Analytic rounded-box SDF plus a gather loop over `NeonConfig::numSamples` perimeter samples (positions in a UBO), reading three baked LUT textures: `uGradientLUT` (base colour ring), `uSegmentLUT` (per-segment gradient atlas, one row per segment), `uArcLUT` (per-arc atlas). All LUTs are baked on the CPU as **RGBA8** - float textures are deliberately avoided for edge-device compatibility. Also owns the opaque-fill pass (`black-rect.frag`).
  ...
  **The glow and the fill have SEPARATE cutoffs.** `NeonConfig::insideCutoff` / `outsideCutoff` (a `Cutoff` each: enable, size, softness) bound the EMISSION and size the draw quad; `opaqueInsideCutoff` / `opaqueOutsideCutoff` (an `OpaqueCutoff` each: enable, size only) bound the FILL, whose single shared feather stays `opaqueSoftness` - which is why `OpaqueCutoff` has no `softness` field rather than one the pass ignores. They were one pair until the split, so a config that bounded its fill by enabling the glow cutoffs no longer does, and `opaqueMode = BOTH` with neither opaque cutoff enabled (their default) fills the whole viewport via the `glClear` path. The two feed different dirty gates - `geometryDirty` watches the glow's outside cutoff, `fillDirty` the opaque pair - and different uniforms: `uInsideCutoff` / `uOutsideCutoff` on `neon.frag` against `uOpaqueInsideCutoff` / `uOpaqueOutsideCutoff` on `black-rect.frag`. In C, the fill's pair is `el_effect_set_opaque_cutoff(effect, side, enable, size)`, addressed by `el_cutoff_side_e`; `el_effect_set_inside_cutoff` / `el_effect_set_outside_cutoff` keep their signatures and now mean the glow only.
...
To add a renderer, subclass `BaseRenderer` (`Initialize` / `Update` / `Render` / `OnConfigChanged`). **`time` arrives as a `double` and must be reduced before it reaches a float uniform** - a raw clock time large enough stops resolving a frame of motion (the neon ring stuttered on 44 frames in 60 at 278 hours of uptime). `TimeUtils::WrapHueTime` does it exactly for anything that uses time as a PHASE; a shader that uses it as an unbounded SCROLL has no period to reduce at and must be given one instead - `droplets.frag` is the worked example, with `DROPLET_PHASE_PERIOD` and `DROPLET_CYCLE_CELLS` locked together in `droplets-tuning.h`. See `docs/review-findings.md` I33b and I35 - and note the build now carries `-Wimplicit-float-conversion` on both library targets precisely so a `float` parameter cannot swallow this again. Add a sub-config struct to `Config` with `operator==`, register it in `demo/src/main.cpp`, and add an ImGui section in `DebugUI`.
...
The effect embeds the manager, so the host does **not** hand-composite animations: attach via `effect.Attach(anim)`, call `anim->Play()`, and each `Update` rebuilds the active config as base + overlays. Read the composited result with `GetActiveConfig()` (useful for UI sliders that should follow animated values); `GetConfig()` returns the untouched base.

**A STOPPED animation does not stop writing.** `AnimationState` is about time (elapsed stops advancing); what a stopped animation writes is `EndAction`, and four of the five keep writing something - so the base value stays hidden underneath and `SetConfig` edits to that field are invisible. That is deliberate: it is what makes a completed fade hold its final value instead of popping back on the last frame. `EndAction::HOLD_NONE` is the one that writes nothing, letting whatever else would have written the field show through - usually the base, but a later animation in attach order if one also writes it, so it steps aside rather than restoring the base. It stays attached and replayable; `Detach` does the same from the outside and is right when the animation is finished with entirely.
...
- **Every effect entry point takes the handle's `dataMutex`, and exactly one does not.** The prologue is two macros: `VALIDATE_EFFECT_PTR(effect, fn)` then `LOCK_EFFECT(effect)`, in that order (the lock dereferences the handle) and both before `VALIDATE_EFFECT_READY`, which reads the `impl` the lock protects. `el_effect_render` is the single entry point with no `LOCK_EFFECT`, because it touches only the renderers and one atomic snapshot cell: **a host thread must never be able to stall a frame.** Its missing lock is called out in a comment at the call site, since an absence is otherwise invisible. `el_effect_destroy` is the other non-locker, for the unrelated reason that it cannot lock a mutex it is about to destroy. The mutex is RECURSIVE for exactly one reason: **host callbacks run with it held.** An animation's `OnComplete` fires synchronously from inside `impl->Update`, inside `el_effect_update`, so anything the host calls from there re-locks on the same thread - and that pattern is deliberately supported (`AnimationManager::Update` walks a copy specifically so "detach me when I finish" is safe). A plain mutex would turn a documented pattern into a hang. Note what is *not* a reason, since both look like re-entry and neither is: `el_effect_init` delegates to `el_effect_init_with_renderers` **without** locking first, and the `el-deprecated.cpp` shims that delegate do not lock either. The ABI never re-enters itself. Firing callbacks after unlocking would remove the need for recursion entirely and is the intended end state. `VALIDATE_EFFECT_READY` is the companion guard for the calls that dereference `impl`; without it an uninitialised handle crashed the host in update, render, capture and every clock and animation call.
- **An attached animation ADOPTS its effect's lock.** `el_animation_handle_impl` carries a `shared_ptr<recursive_mutex>` that `el_effect_attach_animation` sets and `el_effect_detach_animation` clears, and `LOCK_ANIMATION` takes it (a no-op while detached). It has to be the *effect's* mutex, not one of the animation's own: `el_effect_update` reads and writes the very elapsed/state/mode these calls touch, so only the same mutex excludes it - and one mutex rather than two is what makes an `el_animation_*` call from inside a host callback re-enter rather than deadlock. The effect's mutex is therefore **heap-allocated and shared**, because `el_effect_detach_all_animations` never sees handles and so cannot release adoptions; as a plain member that would be a use-after-free after `el_effect_destroy`. Two gaps stay documented rather than enforced: writing the adoption is not atomic (do not attach/detach a handle while another thread calls on it), and one handle serves one effect. Modulators adopt nothing - `el_modulator_sequence_append` is construction-time only, since one modulator can be bound across several effects and there is no coherent lock to take.
- **`el_effect_begin_batch` / `el_effect_end_batch` hold that lock across host code**, which makes a GROUP of calls atomic against `el_effect_update` - a colour-stop rewrite that is one count setter plus N index setters, or a `read_count` followed by indexed reads. They are the only place in the ABI that locks and unlocks manually rather than through a scope guard, because the scope being held is the host's and spans two calls. `end_batch` checks an `atomic<thread::id>` owner **before** unlocking and refuses an unmatched or wrong-thread close: releasing a `recursive_mutex` this thread does not hold is undefined behaviour, not a recoverable error. An open scope blocks `el_effect_update`, never `el_effect_render`, so a leaked one stalls animation and not the frame rate; `el_effect_destroy` unwinds any scope the destroying thread still holds, because the mutex is shared and would otherwise stay locked forever for every attached animation.
...
- A handle therefore reaches **three** configs: staging, base (`GetConfig`) and active (`GetActiveConfig`). The **`el_effect_read_*` family** takes an `el_config_source_e` naming which one to read, addressed by the same field enums `el_animation_add_*_field` binds with - `el_effect_get_intensity(e,&v)` is exactly `el_effect_read_field(e, EL_CONFIG_SOURCE_STAGING, EL_FIELD_NEON_INTENSITY, &v)`. `EL_CONFIG_SOURCE_ACTIVE` is the only way a C host can observe an attached animation's output, and `el_effect_read_count` measures the variable-length containers per source because an animation can grow them past base. **ACTIVE is what renderers are handed, not what they draw** - enable flags, the shader caps and the preserved-first segment merge all still sit between it and the pixels, which is why `EL_CONTAINER_EFFECTIVE_SEGMENTS` exists. **New read surface goes here, not as another named getter.** The switches behind it are the `Read*` / `Write*` pair in `lib/include/animation/field-access.h`, shared with `FieldBoundAnimation` - add a field to one and add it to the other, or a reader will disagree with its writer about what is in range.
