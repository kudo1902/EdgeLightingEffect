...
- [`docs/corner-crease-and-filament-nyquist.md`](docs/corner-crease-and-filament-nyquist.md) - two measured defects and their fixes: the dark diagonal wedges at the corners (halo and bloom were the field of ONE infinite edge, now a sum of four finite segments) and the `resolutionScale` 0.5 mismatch at thin line widths (the filament's floor was in the wrong units). Includes the per-edge bloom pedestal the first fix forced, the small-rect brightness change it causes, and the six offscreen probes behind every number. Read before touching the halo/bloom or filament blocks.
...
- `SpotlightRenderer` - freely placed and aimed cones of light, as ONE additive pass. The odd one out: it is **not a perimeter effect** and reads nothing but `Config::spotlight` - a lamp sits at a `SpotLight::position` in **app coordinates (origin top-left, +y down**, the same space as `RectGeometry::position`) and points at a `SpotLight::angle` in degrees, 0 = right, increasing clockwise. Moving the rect therefore does NOT move the rig. Light only: no backdrop, no fixture housings, no capture, no LUT, no resolution scale, and nothing occludes a cone. Light only, but NOT alpha-free: the pass is additive in ALPHA as well as colour (`glBlendFuncSeparate(GL_ONE, GL_ONE, GL_ONE, GL_ONE)`) and the shader writes the same max-of-channels coverage neon and the flare do. It used to emit a literal alpha 0, which costs nothing on an opaque desktop window and erases the entire layer on an embedded surface a compositor or hardware video plane blends by alpha - the Tizen "spotlight missing over a playing video" bug. Any new layer here must write a coverage alpha for the same reason.
  ...
  **The VBO holds app coordinates verbatim**; the y-flip lives in a flipped ortho (`glm::ortho(0, w, h, 0, ...)`), so the buffer depends only on `Config::spotlight` and a resize costs one uniform rather than a rebuild. The flip reverses winding, which `buildStrips` compensates for by winding its triangles the other way round; it used to be written off as harmless "because nothing here enables `GL_CULL_FACE`", which was a claim about the library where the host owns the state. `EdgeLightingEffect::Render` now takes a `GLUtils::NoCullScope` around the whole fan-out, so no layer depends on the host's cull configuration at all. It takes a `GLUtils::CompositeStateScope` beside it for the rest of the state this library assumes but never sets - a full colour+alpha write mask, `GL_FUNC_ADD`, and depth test/write off - because on a shared surface view a video pipeline or web engine leaves its own behind. Measured: a host alpha write mask zeroes the neon emission pre-pass (whose alpha channel IS the arc weight) and every layer's coverage alpha, and a stray `GL_MAX` changes the composite. **Any new GL state a layer depends on but does not set belongs in one of those two scopes, not in the layer.** Rebuilds happen in `OnConfigChanged`, into a ceiling-sized `GL_DYNAMIC_DRAW` allocation made once in `Initialize`. See [`docs/spotlight-renderer-plan.md`](docs/spotlight-renderer-plan.md) for the design and the verification.
