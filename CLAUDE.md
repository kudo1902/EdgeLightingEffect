...
Shader sources under `lib/shaders/*.{vert,frag}` are read by `lib/CMakeLists.txt` and substituted into `shaders.h.in` via `configure_file()`, producing `build/lib/generated/shaders.h` with each shader as a `const char* const` raw string literal in `EdgeLighting::ShaderSource::*`. There is no runtime file I/O for shaders. `@GLSL_VERSION@` supplies the version line and four tuning headers are injected so their constants are shared between the shaders and the C++ renderers: `@NEON_TUNING@` injects `lib/include/renderer/neon-tuning.h` into `neon.frag`, `neon-emission.frag` and `neon-blit.frag` (the blit needs it for the output-dither constants, which must not drift from the gather's), `@DROPLETS_TUNING@` injects `lib/include/renderer/droplets-tuning.h` into `droplets.frag`, and `@LENS_FLARE_TUNING@` injects `lib/include/renderer/lens-flare-tuning.h` into `lens-flare.frag`, and `@SPOTLIGHT_TUNING@` injects `lib/include/renderer/spotlight-tuning.h` into `spotlight.frag`.

**Comments are stripped on the way in, not on disk.** `read_shader_source` - the one function every shader and tuning header is read through - removes `//` comments before the text reaches `configure_file`, so the files stay as commented as they are and the embedded copies carry none. That is worth 209,952 bytes: `shaders.h` goes 254,719 -> 44,767, and `neon-renderer.cpp.o`'s `__cstring` section 190,693 -> 30,920 (it was 70% of the whole library's string data, because the four neon shaders embedded 188 KB to deliver 27 KB of GLSL, and `neon-tuning.h` - 32,393 bytes carrying 1,199 bytes of `#define` - is injected TWICE). The saving is `.rodata` in every binary plus the text the driver's GLSL compiler lexes at `Initialize`.

Two properties of that strip are load-bearing:

- **Line count is preserved** - a stripped line goes empty, it does not disappear. Costs 3 KB against the 209 KB, and buys that every line number in a driver's shader error log still points at the same line it did before, so an error reads against the commented file on disk. Collapse the blanks as well and every line number shifts by an amount that depends on where in the file the error is.
- **Only `//` is stripped.** No shader or tuning header uses `/* */`, and one that did would simply survive into the embedded copy - valid GLSL, costs bytes, breaks nothing. That is the safe direction; a multi-line handler with its state wrong would eat code instead. Safe on GLSL specifically because the language has no string literals and no line continuations, so `//` can only ever start a comment.

`-DEDGE_LIGHTING_STRIP_SHADER_COMMENTS=OFF` turns it off, which regenerates `shaders.h` byte-identical to the pre-strip one - the A/B switch the verification used. Verified two ways: every embedded shader is token-identical with the comments and whitespace normalised away, and fifteen offscreen scenes covering all five renderers (both resolution paths, all glow sides, the cutoffs, opaque modes, debug overlays) render byte-identical on both sides.

`CMAKE_CONFIGURE_DEPENDS` lists every shader file *and* all four tuning headers, so editing any of them triggers a re-configure on the next build. **If you add a new shader you must update three places**: `lib/CMakeLists.txt` (both the `CMAKE_CONFIGURE_DEPENDS` and `read_shader_source` lists) and `lib/shaders/shaders.h.in`. Read it through `read_shader_source`, not `file(READ ...)`, or it ships its comments.
...
- `NeonRenderer` - the neon stroke. Analytic rounded-box SDF plus a gather loop over `NeonConfig::numSamples` perimeter samples (positions in a UBO), reading three baked LUT textures: `uGradientLUT` (base colour ring), `uSegmentLUT` (per-segment gradient atlas, one row per segment), `uArcLUT` (per-arc atlas). All LUTs are baked on the CPU as **RGBA8** - float textures are deliberately avoided for edge-device compatibility. Also owns the opaque-fill pass (`black-rect.frag`).
  ...
  **Dithers its 8-bit output** (`OUTPUT_DITHER_LSB` in `neon-tuning.h`). The halo and bloom are very low-slope gradients - the bloom falls as `1/D` - so they cross a quantisation step only every few pixels and without this the output is concentric contour rings. The rule is **dither at every 8-bit quantisation, not at the final write**: the scaled path has two (the gather's write into the reduced buffer, and `neon-blit.frag`'s write to the target) and needs both, because dithering only at the blit does *nothing* - by then it is filtering exact multiples of 1/255 and there is no sub-LSB information left to recover. Mean plateau halves on every config; see `docs/review-findings.md` R7 for the tables and the contrast-stretched before/after. Two things to keep in mind before touching it: the value being dithered is a **coverage**, so the noise is faded in by `smoothstep(DITHER_FADE_LO, DITHER_FADE_HI, alpha)` to guarantee no fragment crosses the 0/1 boundary in either direction - an ungated dither moved 18,880 px out of black on the default config, which on a compositing surface is the layer occluding video where it did not before. And set `OUTPUT_DITHER_LSB` to 0.0 before comparing captures against an undithered build: it restores byte-identical output, and without it every frame-diff has a 1 LSB noise floor (2 LSB on the scaled path, which dithers twice by construction).
...
`lib/include/gl/` provides move-only RAII wrappers: `ShaderProgram` (with uniform-location and last-value caching), `VertexArray`, `Framebuffer`, `UniformBuffer`, and `Texture` (base) + `Texture2D`.

**Three of them skip redundant uploads, and a caller should count on it rather than carrying its own dirty flag.** `ShaderProgram::SetUniform` caches the last value per location; `UniformBuffer::SetData` and `VertexArray::SetVertexData` both memcmp the bytes and return without touching GL when nothing moved. That last one is why `setupGeometry` can stay unconditional under an animation: its dirty set is five `AnimatableField`s, but under `GlowSide::INSIDE` or an enabled `outsideCutoff` the quad it computes is pinned to a constant, so a 60 fps intensity sweep went from 119 `glBufferData` calls per 120 frames to **zero** while the case where the quad genuinely moves is untouched. The comparison is on the produced BYTES, never a snapshot of the values they came from - a snapshot is the `operator==`-misses-a-field hazard again, with stale geometry as the symptom. `ShaderProgram`'s location map is a `std::map` with a transparent comparator, not an `unordered_map`, so a `const char *` lookup cannot allocate; see the note there before changing it back. See `docs/review-findings.md` I22 to I24. `lib/include/util/gl-utils.h` adds the scope guards for host-owned state: `NoScissorScope` (per offscreen pass), `NoCullScope` and `CompositeStateScope` (both available but **taken by nothing** - `EdgeLightingEffect::Render` no longer takes them, so their state is the host's to supply). `gl-header.h` is the single include for `<glad/glad.h>`. Use these wrappers - do not call `glGen*` / `glDelete*` directly in renderer code. A multi-pass renderer must return to the framebuffer it was handed (`Framebuffer::GetBoundId()` before its offscreen pass, `Framebuffer::BindId(prev)` after), not to `BindDefault()`: during a frame capture the "backbuffer" is a real FBO.
