...
- [`docs/capi-threading-design.md`](docs/capi-threading-design.md) - the data thread / render thread split for the C ABI (`EL_THREADING_SPLIT`), what it does and does not move, and the ownership table. Read it before assuming `el_effect_update` is a CPU phase - it issues GL calls, which is what rules out the obvious update/render seam. Section 7.4 is the one hazard review missed and ThreadSanitizer caught.
...
- **Two threading models**, chosen per effect by `el_effect_set_threading_mode` before `el_effect_init` and immutable after. `EL_THREADING_SINGLE` is the default and is the historical path byte for byte - no mutex, no queues, `el_effect_publish` a no-op. `EL_THREADING_SPLIT` moves the staging config to a data thread and leaves everything behind the handle on the render thread; the data thread calls `el_effect_publish` / `el_effect_poll_callbacks` / `el_effect_shutdown`, the render thread keeps `init` / `update` / `render` / `destroy`. Animations stay frame-locked on the render thread, so timing is unchanged - the price is that mutating an ATTACHED animation queues a command instead of applying in place. Note the split is **not** update-versus-render: `el_effect_update` uploads LUTs and VBOs through `OnConfigChanged` and is a GL phase. See [`docs/capi-threading-design.md`](docs/capi-threading-design.md).
...
`demo-capi` also takes **`--threaded`**, which runs the same UI against `EL_THREADING_SPLIT`: the main thread is the DATA thread (GLFW windowing and event polling are main-thread-only on macOS, and the ImGui window is where config is authored) and a spawned render thread owns the main window's GL context, `el_effect_init` included. The two windows' existing shared contexts become one per thread. Demo-side state the render thread reads - framebuffer size, background toggles, backdrop texture, rect geometry - is published under one mutex and snapshotted whole; the library's mailbox covers the config, not the host's own state. Both modes share one `DrawMainWindow`, so the flag changes which thread calls it and nothing else.
