...
- [`docs/droplets-region-comparison.md`](docs/droplets-region-comparison.md) - the droplet region as two independent rounded rects of the layer's own, what the perimeter band cost when it moved there, and the two cases that are deliberately NOT the same any more (a wide INSIDE band, and one wider than its corner radius). Read before touching `ResolveRegion` or `droplets.frag`'s region block.
...
Shader sources under `lib/shaders/*.{vert,frag}` are read by `lib/CMakeLists.txt` and substituted into `shaders.h.in` via `configure_file()`, producing `build/lib/generated/shaders.h` with each shader as a `const char* const` raw string literal in `EdgeLighting::ShaderSource::*`. There is no runtime file I/O for shaders. `@GLSL_VERSION@` supplies the version line and three tuning headers are injected verbatim so their constants are shared between the shaders and the C++ renderers: `@NEON_TUNING@` injects `lib/include/renderer/neon-tuning.h` into the neon shaders, `@LENS_FLARE_TUNING@` injects `lib/include/renderer/lens-flare-tuning.h` into `lens-flare.frag`, and `@SPOTLIGHT_TUNING@` injects `lib/include/renderer/spotlight-tuning.h` into `spotlight.frag`. There used to be a fourth, `droplets-tuning.h`: it held one constant, the margin the droplet ring and the shader's discard had to agree on, and the region rework left that margin provably zero - so the header was retired rather than left injected with nothing to say.
...
- `DropletsRenderer` - rain-on-glass droplets in a region of **its own geometry**; screen-space gravity, self-lit drops, no framebuffer capture. The odd one out among the perimeter layers: it reads neither `Config::geometry` nor `NeonConfig::glowSide`, so moving or resizing the glow does not move the rain. A host that wants them to agree copies the numbers across - both demos have a button for it, and seed the band at startup. The one read of another layer's config is `neon.glowSideSoftness`, which feathers the region's boundary: a look knob, not geometry. Draws a **region-fitted ring** - four strips bounding the region itself - rather than a fullscreen quad, so the pass costs what the region costs rather than what the display costs.

  **The region is two rounded rects - `DropletsConfig::outer` minus `inner`.** `RegionDepth` is `min(-sdOuter, sdInner)`, the standard SDF subtraction negated; `hasInner` drops the second term, and "is there an inner shape" IS the band-versus-fill distinction, so there is one concept rather than two. A perimeter band is the concentric special case: `inner` on the rect, `outer` dilated by the thickness - half extents AND corner radius, which is what keeps the two boundaries a constant distance apart.

  **A degenerate inner shape is rejected in `ResolveRegion`, and that guard is load-bearing**: a 0x0 `RectGeometry` is not an empty set to a distance field - `sdRoundBox` with zero half extents is `length(p)`, a POINT - so `min()` read it as a real boundary, carving a phantom hole AND, since the local width is `dOut + dIn`, varying the span radially around it. That cost 23% of a scene's lit pixels with differences 300px from a shape with no size, while `setupGeometry` correctly cut no hole - the two disagreed. **The two shapes are independent**, which is what makes a band of varying thickness, an off-centre hole or a pane with a bite out of it expressible. The price is two full `sdRoundBox` evaluations where a concentric pair could share one `q` - up to 10 per fragment, since `RegionFade` runs 3x per `Drops` call and `Drops` runs up to 3x per fragment. A concentric fast path is possible and deliberately not taken. The off-centre cases are also why `setupGeometry`'s four strips take four independent sides rather than a centred pair of half-extents.

  Two consequences worth knowing. **Drop pitch is its own knob** (`dropSize`): it has to be one GLOBAL px scalar, because every region-relative term in the shader is an amplitude and never a position, precisely so the screen-space grid never shears - and the region's local width is not even constant once the shapes stop being concentric. And **orientation is a property of a curve**: `runs` is forced to 1 for a filled shape rather than computed (an interior fragment has no local run direction, and computing one gives a constant ~0.5 - all beads, no streaks), and for a ring it is read off the OUTER shape, the only one that can speak for the region's orientation once the inner may sit anywhere.

  The mask is a single ramp on the **nearer** boundary's px depth, not the product of two normalised ramps it used to be. Those are the same function wherever the two ramps cannot overlap, which the existing clamps (`soft` at 0.5, the fade radius at 0.45 of the span) already guarantee - **so those two clamps are load-bearing for that equivalence**, not just for their original purposes. The early bail now keeps **no margin at all**, which is exact rather than a tightening: the feather is a hard zero at depth 0, so nothing outside the region is ever written. That is what retired `droplets-tuning.h`.

  **The boundary feather is sized to the DROP, not to the band** - `min(max(glowSideSoftness, dropPitch * 0.25), gSpan * 0.5)`. Its job is to vignette out drop OVERHANG rather than cut it, so the drop is the right scale; the band was only ever an exact stand-in for it while drop size was derived from band width, and decoupling `dropSize` broke that. Sizing off the band also made the feather depend on the INNER shape at the OUTER edge, because `gSpan` is `dOut + dIn` - measured at **11.7x** dimmer, same distance from the same edge, purely from moving the hole. Four forms were built and rendered before this one: it is the only one byte-identical to the shipping band at one lane. Multi-lane bands DO change - they get a proportionally narrower feather, which is the correction. `gSpan` still reaches `runs`, the trail cut and `RegionFade`'s clamp, so independence is ~96%, not total. See section 1.6 of [`docs/droplets-region-comparison.md`](docs/droplets-region-comparison.md).

  The strips must tile without overlapping: this pass blends premultiplied, so a double-covered pixel composites twice. The other invariant is in the shader: the height-field gradient costs two more full evaluations of the droplet field, so it is **gated on `c.x > 0`** - exact, because `c.x = 0` annihilates both terms the normal reaches (`rim` and `spec`). Ungate it and the pass costs ~1.6x more for identical output. **No resolution scale**, and the reason is now only the second of the two it used to be. "This pass never shades the whole viewport" stopped being true when the region became the host's own geometry - a filled `outer` can be any size, and at 1920x1080 it rasterises **30x** what a 24 px band does, so the cost case genuinely exists (and unlike the spotlight's, its blit could be bounded to the region's box rather than the viewport, so the arithmetic closes). What disqualifies it is the other half: the rims and speculars are single-pixel features. Measured against a downsample/upsample stand-in that keeps full-res SHADING and loses only the blit - an optimistic bound, since a real scaled path also samples the droplet field at half rate - scale 0.5 takes the peak from 255 to 166 and leaves **zero** pixels at or above 200, from 27 on a band and 263 on a pane. The layer's whole legibility is in those pixels. The knob that does control this layer's cost is the region itself, which the host now sets directly.
