...
- [`docs/review-findings.md`](docs/review-findings.md) - open defects and rough edges, visual ones with offscreen repros. Check here before assuming a behaviour is intended. Seven review passes; the seventh (I28-I32) covers the threaded-model branch - all fixed, but read I28 before adding a LUT (re-init must `Invalidate` or the bake guards silently skip it) and I29 before adding an `AnimationGroup` override.
