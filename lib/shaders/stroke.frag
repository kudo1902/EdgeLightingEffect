...
void main() {
    ...
    if (uStrokeAnimation == 1) {
        ...
        if (diffPix <= bodyEndPix) {
            ...
            float bodyFade = 1.0 - smoothstep(bodyEndPix - tailSoftPix, bodyEndPix, diffPix);
            finalAlpha = lineAlpha * bodyFade;
        } else {
            float forwardPix = totalP - diffPix;
            float capDist = sqrt(forwardPix * forwardPix + edgeDist * edgeDist);
            if (capDist > limit) discard;
            float capIntensity = 1.0 - smoothstep(0.0, limit, capDist);
            finalAlpha = max(lineAlpha, capIntensity);
        }
    }
    ...
    fragColor = uColor * (finalAlpha * uIntensity) + vec4(vec3(dither), 0.0);
}
