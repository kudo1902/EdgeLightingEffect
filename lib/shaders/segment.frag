...
void main() {
    ...
    if (uEdgeStripWidth > 0.0) {
        if (uEdgeStripSide == 1) effectiveInnerW = uEdgeStripWidth;
        else if (uEdgeStripSide == 2) effectiveOuterW = uEdgeStripWidth;
        else { effectiveInnerW = uEdgeStripWidth * 0.5; effectiveOuterW = uEdgeStripWidth * 0.5; }
    }
    ...
    if (maxTailIntensity <= 0.0) discard;
    ...
    float depthFade = exp(-absD * 0.01);
    float totalGlow = (core * 0.6 + (innerVal + outerVal) * 0.4) * maxTailIntensity * depthFade * pulseIntensity * edgeMask * stripClip;
